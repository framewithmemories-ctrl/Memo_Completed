...
async def admin_recovery_page():
    return HTMLResponse("""<!doctype html><html><head><meta charset='utf-8'><title>Memories Admin Recovery</title><meta name='viewport' content='width=device-width,initial-scale=1'><style>body{font-family:Arial,sans-serif;background:#f7f3ef;margin:0;padding:32px;color:#292522}.card{max-width:520px;margin:40px auto;background:#fff;padding:28px;border-radius:16px;box-shadow:0 8px 30px #0001}h1{margin-top:0}label{display:block;margin:16px 0 6px;font-weight:600}input{width:100%;box-sizing:border-box;padding:12px;border:1px solid #ccc;border-radius:8px}button{margin-top:20px;width:100%;padding:13px;border:0;border-radius:8px;background:#d95f8a;color:#fff;font-weight:700;cursor:pointer}.note{font-size:13px;color:#666}</style></head><body><div class='card'><h1>Memories Admin Recovery</h1><p class='note'>Use the recovery secret configured in Render to set a new admin password.</p><form method='post' action='/api/admin/recover'><label>Admin username</label><input name='username' value='admin' autocomplete='username' required><label>Recovery secret</label><input name='recovery_secret' type='password' autocomplete='off' required><label>New password</label><input name='new_password' type='password' autocomplete='new-password' minlength='12' required><label>Confirm new password</label><input name='confirm_password' type='password' autocomplete='new-password' minlength='12' required><button type='submit'>Reset Admin Password</button></form></div></body></html>""")
