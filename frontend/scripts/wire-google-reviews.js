...
const GOOGLE_STORE_PROFILE_JS = JSON.stringify(GOOGLE_STORE_PROFILE_URL);
...
for (const oldUrl of oldStoreUrls) text = text.replaceAll(oldUrl, GOOGLE_STORE_PROFILE_URL);

text = text.replace(
  /onClick=\{\(\) => window\.open\('https:\/\/www\.google\.com\/maps\/place\/Memories[^']*', '_blank'\)\}/,
  `onClick={() => { window.location.href = ${GOOGLE_STORE_PROFILE_JS}; }}`
);
...
if (text.includes(oldProductFetch)) text = text.replace(oldProductFetch, newProductFetch);
...
if (reviewText.includes(oldReviewLoader)) reviewText = reviewText.replace(oldReviewLoader, newReviewLoader);
...
if (constantsPattern.test(aboutText)) aboutText = aboutText.replace(constantsPattern, constantsReplacement);
...
console.log('Google store wiring applied: verified profile URL is embedded directly; product and review loading retry silently during backend cold starts.');
