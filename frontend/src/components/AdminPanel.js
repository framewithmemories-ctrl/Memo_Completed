...
export const AdminPanel = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <Dialog open={!!orderDetails} onOpenChange={(o)=>!o&&setOrderDetails(null)}><DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Order #{orderDetails?.id?.substring(0,8).toUpperCase()}</DialogTitle><DialogDescription>Complete order information from the existing order record. Nothing is changed by opening this view.</DialogDescription></DialogHeader>{orderDetails&&<div className="space-y-6"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><Card><CardHeader className="pb-3"><CardTitle className="text-base">Customer</CardTitle></CardHeader><CardContent className="space-y-1 text-sm"><p><span className="font-medium">Name:</span> {orderDetails.customer?.name||'Guest'}</p><p><span className="font-medium">Email:</span> {orderDetails.customer?.email||'—'}</p><p><span className="font-medium">Phone:</span> {orderDetails.customer?.phone||'—'}</p>{orderDetails.delivery_address&&<div className="pt-2"><p className="font-medium">Address</p><p className="text-gray-600 whitespace-pre-line">{Object.values(orderDetails.delivery_address).filter(Boolean).join(', ')||'—'}</p></div>}</CardContent></Card><Card><CardHeader className="pb-3"><CardTitle className="text-base">Order & Payment</CardTitle></CardHeader><CardContent className="space-y-1 text-sm"><p><span className="font-medium">Created:</span> {orderDetails.created_at?new Date(orderDetails.created_at).toLocaleString():'—'}</p><p><span className="font-medium">Payment status:</span> <Badge variant="secondary">{orderDetails.payment_status||'—'}</Badge></p><p><span className="font-medium">Payment method:</span> {orderDetails.payment_method||'Not recorded'}</p><p><span className="font-medium">Payment ID:</span> {orderDetails.razorpay_payment_id||'—'}</p><p><span className="font-medium">Order total:</span> <span className="font-semibold">₹{Number(orderDetails.total||orderDetails.total_amount||0).toLocaleString()}</span></p></CardContent></Card></div><Card><CardHeader><CardTitle className="text-base">Items</CardTitle><CardDescription>{orderDetails.items?.length||0} line item(s)</CardDescription></CardHeader><CardContent><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left text-gray-500"><th className="p-2">Product</th><th className="p-2">Variant / Options</th><th className="p-2">Qty</th><th className="p-2">Unit Price</th><th className="p-2">Line Total</th></tr></thead><tbody>{(orderDetails.items||[]).map((item,index)=>{const qty=Number(item.quantity||0);const price=Number(item.price||0);return <tr key={item.id||item.productId||index} className="border-b"><td className="p-2 font-medium">{item.name||item.productName||item.product_id||item.productId||'Product'}</td><td className="p-2 text-gray-600">{renderItemOptions(item)}</td><td className="p-2">{qty}</td><td className="p-2">₹{price.toLocaleString()}</td><td className="p-2 font-medium">₹{(qty*price).toLocaleString()}</td></tr>})}</tbody></table></div></CardContent></Card><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><Card><CardHeader className="pb-3"><CardTitle className="text-base">Totals</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><div className="flex justify-between"><span>Item subtotal</span><span>₹{(orderDetails.items||[]).reduce((sum,item)=>sum+Number(item.price||0)*Number(item.quantity||0),0).toLocaleString()}</span></div><div className="flex justify-between"><span>Store credit applied</span><span>₹{Number(orderDetails.store_credit_applied||0).toLocaleString()}</span></div><div className="flex justify-between border-t pt-2 font-bold"><span>Final total</span><span>₹{Number(orderDetails.total||orderDetails.total_amount||0).toLocaleString()}</span></div><p className="text-xs text-gray-500 pt-2">Promo discount, shipping and tax are shown only when those values are explicitly stored on the order; they are not recalculated here.</p></CardContent></Card><Card><CardHeader className="pb-3"><CardTitle className="text-base">Fulfilment</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><p><span className="font-medium">Delivery type:</span> {orderDetails.delivery_type||'—'}</p><p><span className="font-medium">Order status:</span> <Badge>{orderDetails.order_status||orderDetails.status||'—'}</Badge></p><p><span className="font-medium">Production:</span> {orderDetails.production_status||'—'}</p><p><span className="font-medium">Shipping:</span> {orderDetails.shipping_status||'—'}</p><p><span className="font-medium">Shipping provider:</span> {orderDetails.shipping_provider||'Not recorded'}</p><p><span className="font-medium">Tracking number:</span> {orderDetails.tracking_number||'Not recorded'}</p></CardContent></Card></div><div className="flex justify-end"><Button variant="outline" onClick={()=>setOrderDetails(null)}>Close</Button></div></div>}</DialogContent></Dialog>
