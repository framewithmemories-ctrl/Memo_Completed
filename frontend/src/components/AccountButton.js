...
import { User, Mail, Phone, MapPin, LogOut, Camera, Wallet, Package, Star, Loader2, KeyRound, ShieldAlert, Gift, Eye, EyeOff } from 'lucide-react';
  ...
  return <div>
    ...
    <div className="relative mt-1">
      ...
      <button type="button" onClick={() => setVisible((v) => !v)} className="absolute right-1 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-md hover:bg-rose-50" aria-label={visible ? 'Hide password' : 'Show password'} title={visible ? 'Hide password' : 'Show password'}>
        {visible ? <Gift className="w-5 h-5 text-rose-500" /> : <Gift className="w-5 h-5 text-gray-500" />}
      </button>
    </div>
  </div>;
};
      ...
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto" data-testid="account-dialog">
        {!isAuthenticated ? <>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6 mt-2">
            <TabsList onPointerDown={(e) => e.stopPropagation()} className="grid w-full grid-cols-5"><TabsTrigger value="profile"><User className="w-4 h-4 mr-1" />Profile</TabsTrigger><TabsTrigger value="photos"><Camera className="w-4 h-4 mr-1" />Photos</TabsTrigger><TabsTrigger value="wallet"><Wallet className="w-4 h-4 mr-1" />Wallet</TabsTrigger><TabsTrigger value="orders"><Package className="w-4 h-4 mr-1" />Orders</TabsTrigger><TabsTrigger value="events"><Gift className="w-4 h-4 mr-1" />Important Dates</TabsTrigger></TabsList>
            <div onPointerDown={(e) => e.stopPropagation()}>
              ...
              <TabsContent value="photos"><ProfilePhotoStorage userId={user.id} onPhotoSelected={() => { toast.success('Photo selected!'); setOpen(false); }} /></TabsContent>
