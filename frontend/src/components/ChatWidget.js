...
const getSmartLocalReply = async (text) => {
  ...
  if (/(open|create|make|register|signup|sign up|join).*(account|profile)/i.test(q) || /(personal|user|customer).*(account|profile)/i.test(q) || /(how).*(login|log in|sign in|register)/i.test(q)) {
    return "Absolutely! Memories has customer accounts. 👤 Click the person/account icon in the top-right corner, then choose Create Account and enter your name, email, password and phone number. Once registered, you can use your profile to track orders, save photos, use your wallet/store credits and manage your account. If you already have an account, choose Sign In instead.";
  }
  ...
  if (/(outside|other|elsewhere|not.*memories|from.*other).*(gift|product|item)/i.test(q) || /gift.*(outside|other shops|elsewhere)/i.test(q)) {
    return "I’d love to help you find the closest match from Memories first. 🎁 We specialise in personalised photo frames, customised gifts and sublimation products, so tell me what you need, who it’s for and your budget — I’ll suggest the best options we actually offer.";
  }
  if (/(latest|recent|newest|last).*(review|reviews)/i.test(q) || /(review|reviews).*(latest|recent|newest|last)/i.test(q)) {
    ...
    } catch (e) { return "I’m unable to load the latest review at the moment. Please try again in a moment, or check the Reviews section on the website."; }
  }
  if (/(tomorrow|today|open|closed).*(shop|store|business)/i.test(q) || /(shop|store).*(tomorrow|today|open|closed)/i.test(q)) {
    ...
    return targetWeekday === 'Sunday' ? `No, Boss 😊 Memories is closed on Sunday. We’re open Monday to Saturday, 9:30 AM–9:00 PM. If you meant a different date, tell me the date and I’ll check it for you.` : `Yes — Memories is open on ${targetWeekday}, from 9:30 AM to 9:00 PM. 📍 We’re at 19B Kani Illam, Keeranatham Road, Coimbatore.`;
  }
...
const chooseCatalogueProducts = (products, userText, assistantText) => {
  ...
  const keywordGroups = [
    ['frame', ['frame', 'photo', 'memory', 'picture']],
    ['mug', ['mug', 'coffee', 'cup']],
    ['t-shirt', ['shirt', 't-shirt', 'tee', 'wear']],
    ['corporate', ['corporate', 'office', 'employee', 'bulk', 'company']],
    ['acrylic', ['acrylic', 'modern', 'premium']],
  ];
  const scored = products.map((p, index) => {
    ...
    if (combined.includes((p.name || '').toLowerCase())) score += 20;
    if (budget != null) {
      const price = Number(p.base_price || 0);
      if (price <= budget) score += 4;
      else score -= Math.min(6, (price - budget) / Math.max(100, budget) * 6);
    }
...
export const ChatWidget = () => {
  ...
  useEffect(() => { axios.get(`${API}/products`).then(res => setCatalogue(Array.isArray(res.data) ? res.data : [])).catch(() => setCatalogue([])); }, []);
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user) {
      axios.get(`${API}/chat/history`, authCfg).then((res) => { const hist = res.data?.messages || []; setMessages(hist.length ? [WELCOME, ...hist] : [WELCOME]); }).catch(() => setMessages([WELCOME]));
    } else setMessages([WELCOME]);
  }, [open]);

  const handleClose = () => { setOpen(false); if (!isAuthenticated) { localStorage.removeItem(GUEST_KEY); setMessages([WELCOME]); setRecommendations({}); } };
  ...
  const send = async () => {
    ...
    try {
      ...
      if (!reply) { const res = await axios.post(`${API}/chat`, { session_id: currentSession(), message: text }, authCfg); reply = res.data.reply; }
      ...
      setMessages((m) => [...m, { role: 'assistant', content: "Sorry, I couldn't respond right now. Please WhatsApp us at +91 81480 40148 and our team will help!" }]);
      ...
      {open && (
        <div data-testid="chat-widget-panel" className="fixed bottom-24 right-6 z-40 w-[92vw] max-w-sm h-[70vh] max-h-[620px] bg-white rounded-2xl shadow-2xl border border-purple-100 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-200">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
            ...
            {loading && <div className="flex justify-start"><div className="px-3 py-2 rounded-2xl bg-white border border-gray-200 text-gray-500 flex items-center gap-2 text-sm"><Loader2 className="w-4 h-4 animate-spin" /> Memo is typing…</div></div>}
          </div>
